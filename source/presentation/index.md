...
Content resources are associated with the TextFrame via Web Annotations. We recommend a number of [values for the `motivation`](#values-for-motivation). They can be divided into two broad categories: those where the content of the annotation is _part of_ the TextFrame and those where the content is derived _from_ the TextFrame. Annotations with the motivation value of `writing` would be used to denote text on the original document, such as cataloguer's numbers or scribal edits. Annotations with the motivation value of `asserting`, `classifying`, `identifying` or `facing` are making statements about some aspect of the target. `associating` is used to record content that is associated with the TextFrame, such as IIIF images for the underlying resource, embedded diagrams, different drafts of exemplars, or an audio representation of the target.
